...
// Typescript doesn't require any folder structure like js and we can configure the app as we want

// typescript has pther options as well to transpile:
// transiple my fileName.js file for target ES6 into the output Directory names 'js' 
// >tsc --target ES2015 --outDir js fileName.ts

// To generate an editable tsconfig.json file we run 
// >tsc --init

// after tsconfig.json is setup we can just simply run:
// >tsc
// It will run all files with .ts extension

export {}                           //for treating it as a module rather than a script (having global scope)
...
let person1: [string,number] = ['manav',10]
...
(myVariable2 as string).toUpperCase();  //no error now
...
//When we have more number of properties of an object, we ca simplyfy the code using 'interace'
//We can also make few properties optional using '?' in an interface (here middleName is optional)
interface Person {
    firstName: string,
    lastName: string,
    middleName ? : string               //for optional form fields
}
...
class Manager extends Employee {                //inheritance
    constructor(managerName: string){           //constructor for derived class Manager
...
class Employee2 {
    public empName : string;          //by default public, else we can also specify
    //if 'private' it can not be accessed outside the class but only within the class methods
    
    private code : number;           //can be accessed only within its own class method

    protected key: string = 'abcd123';      //can be accessed within the methods of base and derived classed
...
class Manager2 extends Employee2 {                //inheritance
    constructor(managerName: string){           //constructor for derived class Manager
